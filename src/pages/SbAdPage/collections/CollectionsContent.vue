...
const campaignTableGroups = computed(() => {
  ...
  return campaignKeywordRows.value.map(base => {
    ...
    return { base, lines: types.map(mt => { const s = base.suggestByMatch[mt]; return { rowKey: `${base.id}-${mt}`, matchType: mt, suggestBid: s?.bid ?? '—', suggestRange: s?.range ?? '—' } }) }
...
function addFromRow(row) {
  ...
  form.value.keywords.push({
    ...
    text, matchType: row.matchType,
    is: row.is ?? '—', ir: row.ir ?? '—',
    suggestBid: row.suggestBid ?? '—', suggestRange: row.suggestRange ?? '—',
  ...
  sourceText.split('\n').map(l => l.trim()).filter(Boolean).forEach(line => {
    selectedTypes.forEach(matchType => {
      ...
      form.value.keywords.push({
        ...
        text: line, matchType, is: '—', ir: '—', suggestBid: '—', suggestRange: '—', bid,
      })
    })
  })
}
...
/* ── KT Shell (identical to SP KeywordTargetingPage) ── */
.kt-shell {
  display: grid;
